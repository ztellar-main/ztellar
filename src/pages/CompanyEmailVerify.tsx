...
const CompanyEmailVerify = () => {
  ...
  const companyContactNumber = userData?.companyContactNumber;
  ...
  const submitButtonFunction = async () => {
    ...
    try {
      const res = await axios({
        method: "post",
        url: "/users/company-verify-email-and-signup",
        data: { otp, email, companyName, companyContactNumber, password},
      });
      setLoading(false);
      ...
      window.history.replaceState({}, companyName);
      window.history.replaceState({}, companyContactNumber);
